        ...
        <use href="${f}#icon-menu-burg"></use>
            ...
            <li class="books_list">
                ...
                <ul class="images_books">`;for(let a in e[s].books)if(o+=`<li class="image_book" data-category="${e[s].books[a]._id}">
                            ...
                            </li>`,t==a)break;o+=`</ul><button class="button-see-more" data-category="${e[s].list_name}">See more</button></li>`}g.insertAdjacentHTML("beforeend",o),p.style.display="none"}g.addEventListener("click",async e=>{if(e.target.classList.contains("button-see-more")){p.style.display="block",g.innerHTML="";const o=e.target.dataset.category;n.textContent=o,h.querySelector(".categoria")&&h.querySelectorAll(".categoria").forEach(r=>{n.textContent===r.textContent&&(document.querySelector(".selected-categories").classList.remove("selected-categories"),r.classList.add("selected-categories"))});const t=n.textContent.split(" "),s=t[t.length-1];n.innerHTML=n.innerHTML.replace(s,`<span class="blue">${s}</span>`),await l.get(`https://books-backend.p.goit.global/books/category?category=${o}`).then(a=>{if(a.data.length===0)throw new Error("Sorry, books in the selected category were not found");b(a.data)}).catch(a=>c.error({title:"Error",message:a.message}))}});async function B(){g.innerHTML="",p.style.display="block",n.textContent==="Best Sellers Books"?await l.get("https://books-backend.p.goit.global/books/top-books").then(e=>{if(e.data.length===0)throw new Error("Sorry, books in the selected category were not found");v(e.data)}).catch(e=>c.error({title:"Error",message:e.message})):await l.get(`https://books-backend.p.goit.global/books/category?category=${m}`).then(e=>{if(e.data.length===0)throw new Error("Sorry, books in the selected category were not found");b(e.data)}).catch(e=>c.error({title:"Error",message:e.message}))}B();async function C(e){let o="";const t='<li tabindex="0" data-name="Best Sellers Books" class="categoria selected-categories">All categories</li>';e.forEach(a=>{o+=`<li tabindex="0" data-name="${a.list_name}" class="categoria">${a.list_name}</li>`}),h.insertAdjacentHTML("beforeend",o),h.insertAdjacentHTML("afterbegin",t),document.querySelectorAll(".categoria").forEach(a=>{a.addEventListener("click",async r=>{p.style.display="block",g.innerHTML="",r.preventDefault(),document.querySelector(".selected-categories").classList.remove("selected-categories"),r.target.classList.add("selected-categories"),m=r.target.textContent,m=m.replace(/ /g,"%20"),r.target.textContent!=="All categories"?(n.textContent=r.target.textContent,w(),await l.get(`https://books-backend.p.goit.global/books/category?category=${m}`).then(i=>{if(i.data.length===0)throw new Error("Sorry, books in the selected category were not found");b(i.data)}).catch(i=>c.error({title:"Error",message:i.message}))):(n.textContent="Best Sellers Books",w(),await l.get("https://books-backend.p.goit.global/books/top-books").then(i=>{if(i.data.length===0)throw new Error("Sorry, books in the selected category were not found");v(i.data)}).catch(i=>c.error({title:"Error",message:i.message})))})})}l.get("https://books-backend.p.goit.global/books/category-list").then(e=>{C(e.data)}).catch(e=>c.error({title:"Error",message:e.message}));async function b(e){console.log("ok");let o="";e.forEach(async t=>{o+=`<li class="book" data-category="${t._id}">
                        <div class="div-animation">
